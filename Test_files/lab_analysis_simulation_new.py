...
class Order(BasicEntity):
    ...
    def process(self):
        ...
        while not evaluation_success:
            ...
            self.process_step(
                ...
                already_claimed=True,  # Sorting is pre-claimed in preparation step
                mode="sorting",
    ...
    def process_step(
        ...
        # Claim this server — skip if already pre-claimed by previous step
        ...
        # ── Blocking: pre-claim next server before releasing current one ──
        # request() natively waits until capacity is available — no polling,
        # no race conditions. The order briefly holds both servers at once.
        if next_server is not None:
            self.request(next_server, mode="waiting_for_next")
        # ──────────────────────────────────────────────────────────────────
        ...
        # if next_server is not None:
            ...
            #     self.hold(0.5, mode="blocked")  # kurz warten, dann nochmal prüfen
        #     # self.wait(
        ...
        # print("After wait")
            ...
            #     mode="blocked",
            # )
        # In process_step, statt self.wait():
        # if next_server is not None:
        #     while next_server.available_quantity() <= 0:
        #         next_server.available_quantity_state.waiters().append(self)
        #         self.passivate(mode="blocked")
        #         next_server.available_quantity_state.waiters().remove(self)
        # ───────────────────────────────────────────────────────────────────────
...
if __name__ == "__main__":
    ...
    # ###########################################################################
    print(run_single_scenario(animate=True))
