...
Note: True Hyperparameter Tuning using this file is not recommended as the whole process is very computationally expensive.
This script can be used as an example how the process works an can be run using low episode counts and few evaluation replications to get a rough idea of the tuning process.
For proper hyperparameter tuning, it is recommended to use the ZHAW HPC infrasturcture and the provided SLURM scripts in this repository.
"""
...
RL_ROOT = Path(__file__).resolve().parents[3] / "RL"
for path in (
...
OUTPUT_DIR = BASE_DIR / "results" / "old_unused_deprecated_results" / "rl_bo_results"
...
def run_with_args(args: argparse.Namespace) -> dict[str, Any]:
    if args.smoke_test:
        ...
        if args.output_dir is None:
            args.output_dir = Path("results") / "old_unused_deprecated_results" / "rl_bo_results_smoke"
