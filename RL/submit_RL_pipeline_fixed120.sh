...
if [ ! -f "run_RL_stage1_BO_fixed120.sh" ]; then
    echo "ERROR: run_RL_stage1_BO_fixed120.sh not found in $RL_DIR"
    exit 1
fi

if [ ! -f "run_RL_stage2_array_fixed120.sh" ]; then
    echo "ERROR: run_RL_stage2_array_fixed120.sh not found in $RL_DIR"
    exit 1
fi

echo "Submitting RL Stage 1..."
STAGE1_JOB_ID="$(sbatch --parsable run_RL_stage1_BO_fixed120.sh)"
echo "Stage 1 job submitted: $STAGE1_JOB_ID"

echo "Submitting RL Stage 2 array with dependency afterok:$STAGE1_JOB_ID..."
STAGE2_JOB_ID="$(sbatch --parsable --dependency=afterok:"$STAGE1_JOB_ID" run_RL_stage2_array_fixed120.sh)"
